...
/// TODO
#[derive(Clone)]
pub struct DescriptorChainConsumer<'a, B> {
    buffers: VecDeque<(bool, VolatileSlice<'a, B>)>,
    bytes_consumed: usize,
}

impl<'a, B: BitmapSlice + 'static> DescriptorChainConsumer<'a, B> {
    /// Construct a new Reader wrapper over `desc_chain`.
    pub fn new<M>(mem: &'a GuestMemoryMmap<B>, desc_chain: DescriptorChain<M>) -> Result<Self>
    where
        M: Deref,
        ...
        let buffers = desc_chain
            .map(|desc| {
                ...
                region
                    ...
                    .map(|slice| (desc.is_write_only(), slice))
            })
            .collect::<Result<_>>()?;
    ...
    //     self.buffers
    //         .iter()
    //         .fold(0usize, |count, (_, vs)| count + vs.len())
    // }
    ...
    fn consume<F>(&mut self, count: usize, write_only: bool, f: F) -> io::Result<usize>
    where
        F: FnOnce(&[&VolatileSlice<B>]) -> io::Result<usize>,
    {
        let mut buflen = 0;
        let mut bufs = Vec::with_capacity(self.buffers.len());
        for (writeable, vs) in &self.buffers {
            if *writeable != write_only || buflen >= count {
                break;
            }
        ...
        while let Some((writable, vs)) = self.buffers.pop_front() {
            if rem < vs.len() {
                ...
                self.buffers.push_front((writable, vs.offset(rem).unwrap()));
                break;
            }
    ...
    //     let mut rem = offset;
    //     let pos = self.buffers.iter().position(|(writable, vs)| {
    //         if rem < vs.len() {
    //             true
    //         } else {
    //             rem -= vs.len();
    ...
    //             let (writable, slice) = other.pop_front().expect("empty VecDeque after split");
    //             self.buffers.push_back((
    //                 writable,
    //                 slice.subslice(0, rem).map_err(Error::VolatileMemoryError)?,
    //             ));
    //             other.push_front((
    //                 writable,
    //                 slice.offset(rem).map_err(Error::VolatileMemoryError)?,
    //             ));
    //         }
...
impl<'a, B: BitmapSlice + 'static> io::Read for DescriptorChainConsumer<'a, B> {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        self.consume(buf.len(), false, |bufs| {
            let mut rem = buf;
            let mut total = 0;
            for vs in bufs {
...
impl<'a, B: BitmapSlice + 'static> io::Write for DescriptorChainConsumer<'a, B> {
    fn write(&mut self, buf: &[u8]) -> io::Result<usize> {
        self.consume(buf.len(), true, |bufs| {
            let mut rem = buf;
            let mut total = 0;
            for vs in bufs {
                let copy_len = cmp::min(rem.len(), vs.len());
...
mod tests {
    use super::*;
    use crate::{Queue, QueueOwnedT, QueueT};
    ...
    fn reader_test_simple_chain() {
        ...
        let mut reader =
            DescriptorChainConsumer::new(&memory, chain).expect("failed to create Reader");
    ...
    fn writer_test_simple_chain() {
        ...
        .expect("create_descriptor_chain failed");
        let mut writer =
            DescriptorChainConsumer::new(&memory, chain).expect("failed to create Writer");
    ...
    fn reader_writer_shared_chain() {
        ...
        .expect("create_descriptor_chain failed");
        let mut cursor =
            DescriptorChainConsumer::new(&memory, chain.clone()).expect("failed to create Reader");
    ...
    fn reader_writer_shattered_object() {
        ...
        .expect("create_descriptor_chain failed");
        let mut writer =
            DescriptorChainConsumer::new(&memory, chain_writer).expect("failed to create Writer");
        ...
        .expect("create_descriptor_chain failed");
        let mut reader =
            DescriptorChainConsumer::new(&memory, chain_reader).expect("failed to create Reader");
    ...
    //     )
    //     .expect("create_descriptor_chain failed");
    //     let mut reader = DescriptorChainConsumer::new(&memory, chain).expect("failed to create Reader");
